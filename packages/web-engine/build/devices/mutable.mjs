...
void pd_note_off(float note) {
  // Monophonic, as the module is: only the sounding note's own off ends it - or a negative
  // note, which is all notes off.
  if (note >= 0.0f && note != g_note) return;
  g_state.gate = false;
}
...
// The note sounding, so the off of a note already replaced does not end the one that replaced it.
float g_note = -1.0f;

// Chiptune, played from notes, is an arpeggiator that envelopes itself and so bypasses the
// low-pass gate - and with Timbre Mod at zero its envelope never decays, so a note would sound
// for ever after it ended. That engine alone is given a gate here, with the response the gate
// has in every other engine: Decay and LPG Color set its tail, and it closes when the note does.
const int kChiptune = 7;
plaits::LPGEnvelope g_gate;
float g_gateGain = 1.0f;

float ChiptuneGate() {
  const float decay = g_patch.decay;
  const float hf = g_patch.lpg_colour;
  // The module's own formulas (plaits/dsp/voice.cc), per block of kBlockSize.
  const float short_decay = (200.0f * plaits::kBlockSize) / plaits::kSampleRate *
      stmlib::SemitonesToRatio(-96.0f * decay);
  const float decay_tail = (20.0f * plaits::kBlockSize) / plaits::kSampleRate *
      stmlib::SemitonesToRatio(-72.0f * decay + 12.0f * hf) - short_decay;
  float level = 1.3f * g_mods.level / (0.3f + fabsf(g_mods.level));
  if (level < 0.0f) level = 0.0f;
  if (level > 1.0f) level = 1.0f;
  g_gate.ProcessLP(level, short_decay, decay_tail, hf);
  return g_gate.gain();
}
...
void Generate(int frames) {
  ...
  while (frames > 0) {
    // kBlockSize, not kMaxBlockSize: the module steps its envelopes and its trigger once per
    // render, with coefficients worked out for kBlockSize frames, as the hardware calls it.
    int n = frames > (int)plaits::kBlockSize ? (int)plaits::kBlockSize : frames;
    g_voice.Render(g_patch, g_mods, tmp, (size_t)n);
    const float blend = g_params[4];
    float from = g_gateGain;
    float to = 1.0f;
    if (g_patch.engine == kChiptune) to = ChiptuneGate();
    else g_gate.Init();
    g_gateGain = to;
    for (int i = 0; i < n; i++) {
      if (g_fifoCount >= kFifo) break;
      ...
      const float gain = from + (to - from) * (float)(i + 1) / (float)n;
      g_fifo[g_fifoCount++] = (a + (b - a) * blend) * gain;
    }
    frames -= n;
  }
}
...
void pd_note_off(float note) {
  // Monophonic: the last note on is the one sounding, and only its own off ends it - or a
  // negative note, which is all notes off.
  if (note >= 0.0f && note != g_note) return;
  g_note = -1.0f;
  g_mods.trigger = 0.0f;
  g_mods.level = 0.0f;
