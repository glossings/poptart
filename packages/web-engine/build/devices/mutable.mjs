...
export const PLAITS_PARAMS = Object.freeze([
  ...
  { id: 'harmonics', name: 'Harmonics', min: 0, max: 1, default: 0.5, group: 'Engine', description: "The first of the three model controls. Broadly: how much material there is - the number of partials, the spread of a chord, the depth of the FM. It means something different in every engine." },
  { id: 'timbre', name: 'Timbre', min: 0, max: 1, default: 0.5, group: 'Engine', description: "The second model control. Broadly: brightness, or the balance of what Harmonics set up. It means something different in every engine." },
  { id: 'morph', name: 'Morph', min: 0, max: 1, default: 0.5, group: 'Engine', description: "The third model control. Broadly: the character of the waveform itself, often from soft to hard. It means something different in every engine." },
  {
    id: 'blend',
    name: 'Blend',
    min: 0,
...
export function peaksDescriptor(sourceUrl) {
  return {
    id: 'Peaks',
    kind: 'synth',
    version: 1,
    ...
    channels: { in: 0, out: 2 },
    params: PEAKS_PARAMS.map((p) => ({ rate: 'a', ...p })),
  };
}

/**
 * The C that gives Peaks the device ABI.
 *
 * Its four drums are separate classes with the same shape, so the voice control picks which one
 * renders. They take a gate as a per-sample flag rather than a call, which is how the module
 * reads its trigger input; a note becomes a rising edge on the first sample it lands on.
 */
export function peaksWrapper(maxBlock) {
...
peaks::BassDrum g_bass;
peaks::SnareDrum g_snare;
peaks::HighHat g_hat;
peaks::FmDrum g_fm;
bool g_ready = false;
bool g_rising = false;
