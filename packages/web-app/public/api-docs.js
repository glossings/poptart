...
const API_DOCS = {
  ...
  // ----------------------------------------------------------------- arithmetic
  add: { kind: 'method', sig: 'add(x)', desc: 'Adds x to each value, keeping this pattern\'s rhythm. On notes, .add(2) is two semitones and .add(n(2)) is two steps in the current scale. A stacked value like note("0,7") plays each layer at once.', eg: '.add(n("<0 2 -1>"))' },
  sub: { kind: 'method', sig: 'sub(x)', desc: 'Subtracts x from each value, keeping this pattern\'s rhythm. On notes, .sub(1) is a semitone and .sub(n(1)) is one step in the current scale. A stacked value plays each layer at once.', eg: '.sub(12)' },
  mul: { kind: 'method', sig: 'mul(x)', desc: 'Multiplies each value by x, keeping this pattern\'s rhythm. A stacked value plays each layer at once: .mul(speed("1.1,0.9")) plays two detuned hits.', eg: '.mul(speed(2))' },
  div: { kind: 'method', sig: 'div(x)', desc: 'Divides each value by x, keeping this pattern\'s rhythm.', eg: 'irand(8).div(8)' },
  mod: { kind: 'method', sig: 'mod(x)', desc: 'Remainder after dividing by x, always positive, keeping this pattern\'s rhythm. On notes, .mod(12) folds into one octave of semitones and .mod(n(7)) into one octave of the current scale.', eg: '.mod(12)' },
