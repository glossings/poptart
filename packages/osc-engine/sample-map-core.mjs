// The sample map's maths, shared by both builds: every sample in the chosen folders as a point
// on a plane, near the ones that sound like it, in groups that (mostly) turn out to be kicks, snares, hats and so on.
...
//
//   mixdownHead       the first few seconds of a decoded file as mono (sample-map.js reads
//                     one off the disk; the browser build decodes it)
...
// globally.
//
// This file is the part with no disk in it, as an ES module, so the desktop (sample-map.js, which
// adds reading a WAV's head off the disk) and the browser build (a worker that is handed decoded
// audio) run the same code and draw the same map. Nothing here may reach for node or the DOM.
...
/** The mono head of a decoded file: { sampleRate, channels, frames, totalFrames, data } in. */
...
function nameVotes(filePath) {
  ...
  // Either separator: the desktop hands Windows paths, the browser build "pack/folder/file".
  const segs = String(filePath).split(/[\\/]/).filter(Boolean);
  const file = segs.pop() ?? '';
  const base = file.replace(/(.)\.[^.]*$/, '$1');
  add(nameTokens(base), 1);
  const parts = segs.reverse();
...
// ---------------------------------------------------------------------------------------------
// Queries on a built map
// ---------------------------------------------------------------------------------------------
//
// A built map is deriveMap's result plus the index's bookkeeping beside it: `paths`, `sources`
// (each point's source, as the index names sources) and `seconds`. These answer the panel's
// questions the same way whichever build's index holds the map.

export function round3(v) {
  return Math.round(v * 1000) / 1000;
}

/** What the client draws: every point with its position, type, group and source (an index into `sources`). */
export function mapSnapshot(m, sources) {
  if (!m) return { sources, clusterLabels: [], points: [] };
  return {
    sources,
    ...
    points: m.paths.map((p, i) => ({
      ...
      source: sources.indexOf(m.sources[i]),
    })),
  };
}

/** Point `i`'s nearest neighbors on the map, nearest first, as [{ path, dist, label }]. */
export function mapNeighbors(m, i, k) {
  if (!m || i < 0 || !m.neighbors[i]) return [];
  const { index, dist } = m.neighbors[i];
  const out = [];
  for (let t = 0; t < Math.min(k, index.length); t++) {
    if (index[t] < 0) break;
    out.push({ path: m.paths[index[t]], dist: round3(dist[t]), label: m.labels[index[t]] });
  }
  return out;
}

/** Each point's neighborhood radius (distance to its farthest kept neighbor) and the median. */
export function mapSupport(m) {
  if (!m._support) {
...
/**
 * The point farthest (in feature space) from everything already in the kit - the "give me a
 * unique one" action - as an index, or -1. `chosen` and `skip` are point indices; `types` and
 * `sourceIndices` narrow the candidates.
 *
 * `typical` (default on) keeps the search to well-supported samples - those whose neighborhood
 * is no wider than the map's median. Farthest-point sampling otherwise hands back the library's
 * oddities first (the one gamelan hit, the bullwhip), because an outlier is by definition far
 * from everything; a kit wants the farthest sound that is still a representative of something.
 */
export function mapUnique(m, sources, chosen, { types = null, sourceIndices = null, skip = new Set(), typical = true } = {}) {
  if (!m || !m.points.length) return -1;
  const support = typical ? mapSupport(m) : null;
  const candidates = [];
  for (let i = 0; i < m.paths.length; i++) {
    if (types && !types.includes(m.labels[i])) continue;
    if (sourceIndices && !sourceIndices.includes(sources.indexOf(m.sources[i]))) continue;
    if (support && support.radius[i] > support.median) continue;
    candidates.push(i);
  }
  return farthestFrom(m.points, chosen, { candidates, exclude: new Set([...skip, ...chosen]) });
}

/**
 * Every kit slot hopped to one of its own near neighbors - same roles, different flavor. A slot
 * whose file isn't on the map, or whose neighbors are all already in the kit, keeps its file.
 * `neighborsOf(path)` is the index's own lookup; `rng` is injectable so a test can pin it.
 */
export function mapReshuffle(kit, neighborsOf, { rng = Math.random } = {}) {
  const taken = new Set(kit);
  return kit.map((p) => {
    const options = neighborsOf(p).map((n) => n.path).filter((q) => !taken.has(q));
...
// A built map as JSON for a cache, and back. Float arrays go as base64 of their bytes.
...
export function base64ToF32(text) {
  const bin = atob(text);
...
export function encodeMap(m) {
  return {
    paths: m.paths,
    sources: m.sources,
    seconds: m.seconds,
    points: m.points.map((p) => f32ToBase64(p)),
    xy: f32ToBase64(m.xy),
    ...
    clusterLabels: m.clusterLabels,
    prepared: m.prepared && {
      mean: f32ToBase64(m.prepared.mean),
      scale: f32ToBase64(m.prepared.scale),
      basis: m.prepared.basis.map((b) => f32ToBase64(b)),
    },
  };
}

/** A cached map back as arrays, or null for one that will not read. */
export function decodeMap(raw) {
  try {
    return {
      ...
      sources: raw.sources,
      seconds: raw.seconds,
      points: raw.points.map((p) => base64ToF32(p)),
      xy: base64ToF32(raw.xy),
      ...
      prepared: raw.prepared && {
        mean: base64ToF32(raw.prepared.mean),
        scale: base64ToF32(raw.prepared.scale),
        basis: raw.prepared.basis.map((b) => base64ToF32(b)),
      },
    };
  } catch {
    return null;
...
// ---------------------------------------------------------------------------------------------
// FFT (in place, radix 2; re/im lengths a power of two) - also song-detect.js's
// ---------------------------------------------------------------------------------------------
